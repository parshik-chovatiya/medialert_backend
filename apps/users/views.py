...
def login_view(request):
    ...
    if serializer.is_valid():
        ...
        if not CustomUser.objects.filter(email=email).exists():
            return StandardResponse.unauthorized(message='User is not registered')
...
def password_reset_request_view(request):
    ...
    if serializer.is_valid():
        ...
        return StandardResponse.success(message='If an account with this email exists, a password reset link has been sent.')
...
def password_reset_confirm_view(request):
    ...
    if serializer.is_valid():
        ...
        if user is not None and default_token_generator.check_token(user, token):
            ...
            user.save()
            return StandardResponse.success(message='Password has been reset successfully. You can now login.')
        else:
            return StandardResponse.error(message='Invalid or expired reset token.', status_code=status.HTTP_400_BAD_REQUEST)
